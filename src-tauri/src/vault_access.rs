...
pub fn verify_hash(hash: &str, password: &str) -> bool {
    ...
    Argon2::default()
        .verify_password(password.as_bytes(), &this_hash)
        .is_ok()
}
...
//for context, Argon 2 reccomends using mat_key for storing password, where as the pure_password_hash is directly computed hash without special key function
